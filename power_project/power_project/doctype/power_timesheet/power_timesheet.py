...
class PowerTimesheet(Document):
	...
	def validate_overlap(self, data):
		settings = frappe.get_single("Power Projects Settings")
	...
	def get_overlap_for(self, fieldname, args, value):
		timesheet = frappe.qb.DocType("Power Timesheet")
		timelog = frappe.qb.DocType("Power Timesheet Detail")
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	query = f"""
		...
		FROM `tabPower Timesheet Detail` tsd
			INNER JOIN `tabPower Timesheet` ts
...
def get_timesheet_detail_rate(timelog, currency):
	timelog_detail = frappe.db.sql(
		...
		ts.currency as currency FROM `tabPower Timesheet Detail` tsd
		INNER JOIN `tabPower Timesheet` ts ON ts.name=tsd.parent
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql(
		f"""select distinct tsd.parent from `tabPower Timesheet Detail` tsd,
			`tabPower Timesheet` ts where
...
def make_sales_invoice(source_name, item_code=None, customer=None, currency=None):
	...
	timesheet = frappe.get_doc("Power Timesheet", source_name)
...
def get_events(start, end, filters=None):
	...
	return frappe.db.sql(
		"""select `tabPower Timesheet Detail`.name as name,
			`tabPower Timesheet Detail`.docstatus as status, `tabPower Timesheet Detail`.parent as parent,
			from_time as start_date, hours, activity_type,
			`tabPower Timesheet Detail`.project, to_time as end_date,
			CONCAT(`tabPower Timesheet Detail`.parent, ' (', ROUND(hours,2),' hrs)') as title
		from `tabPower Timesheet Detail`, `tabPower Timesheet`
		where `tabPower Timesheet Detail`.parent = `tabPower Timesheet`.name
			and `tabPower Timesheet`.docstatus < 2
			and (from_time <= %(end)s and to_time >= %(start)s) {conditions} {match_cond}
		""".format(conditions=conditions, match_cond=get_match_cond("Power Timesheet")),
		{"start": start, "end": end},
		as_dict=True,
		update={"allDay": 0},
	)
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	if customer:
		...
		projects = [d.name for d in frappe.get_all("Power Project", filters={"customer": customer})]
		...
		timesheets = frappe.db.sql(
			...
			FROM `tabPower Timesheet` ts, `tabPower Timesheet Detail` tsd
