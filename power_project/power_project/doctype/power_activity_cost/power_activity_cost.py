...
class PowerActivityCost(Document):
	...
	def check_unique(self):
		if self.employee:
			if frappe.db.sql(
				"""select name from `tabPower Activity Cost` where employee_name= %s and activity_type= %s and name != %s""",
			...
			if frappe.db.sql(
				"""select name from `tabPower Activity Cost` where ifnull(employee, '')='' and activity_type= %s and name != %s""",
