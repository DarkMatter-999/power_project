...
class PowerProjectTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from power_project.power_project.doctype.power_project_template_task.power_project_template_task import (
			ProjectTemplateTask,
		)
	...
	def validate_dependencies(self):
		for task in self.tasks:
			task_details = frappe.get_doc("Power Task", task.task)
			if task_details.depends_on:
				for dependency_task in task_details.depends_on:
					if not self.check_dependent_task_presence(dependency_task.task):
						task_details_format = get_link_to_form("Power Task", task_details.name)
						dependency_task_format = get_link_to_form("Power Task", dependency_task.task)
						frappe.throw(
