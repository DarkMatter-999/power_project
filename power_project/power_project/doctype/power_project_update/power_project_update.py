...
class PowerProjectUpdate(Document):
	...
	if TYPE_CHECKING:
		...
		from power_project.power_project.doctype.power_project_user.power_project_user import PowerProjectUser
...
def daily_reminder():
	project = frappe.db.sql(
		"""SELECT `tabPower Project`.project_name,`tabProject`.frequency,`tabProject`.expected_start_date,`tabProject`.expected_end_date,`tabProject`.percent_complete FROM `tabProject`;"""
	)
	for projects in project:
		...
		progress = projects[4]
		draft = frappe.db.sql(
			"""SELECT count(docstatus) from `tabPower Project Update` WHERE `tabProject Update`.project = %s AND `tabProject Update`.docstatus = 0;""",
			project_name,
		)
		for drafts in draft:
			number_of_drafts = drafts[0]
		update = frappe.db.sql(
			"""SELECT name,date,time,progress,progress_details FROM `tabPower Project Update` WHERE `tabProject Update`.project = %s AND date = DATE_ADD(CURRENT_DATE, INTERVAL -1 DAY);""",
			project_name,
		)
...
def email_sending(project_name, frequency, date_start, date_end, progress, number_of_drafts, update):
	...
	msg += "</table>"
	if len(holiday) == 0:
		email = frappe.db.sql("""SELECT user from `tabPower Project User` WHERE parent = %s;""", project_name)
		for emails in email:
