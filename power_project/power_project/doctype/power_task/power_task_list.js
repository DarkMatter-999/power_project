frappe.listview_settings["Power Task"] = {
	add_fields: [
		"project",
		"status",
		"priority",
	...
	onload: function (listview) {
		var method = "power_project.power_project.doctype.power_task.power_task.set_multiple_status";
	...
	gantt_custom_popup_html: function (ganttobj, task) {
		let html = `
			<a class="text-white mb-2 inline-block cursor-pointer"
				href="/app/powertask/${ganttobj.id}"">
				${ganttobj.name}
			</a>
		`;
		...
		if (task.project) {
			html += `<p class="mb-1">${__("Project")}:
				<a class="text-white inline-block"
					href="/app/powerproject/${task.project}"">
					${task.project}
				</a>
			</p>`;
		}
