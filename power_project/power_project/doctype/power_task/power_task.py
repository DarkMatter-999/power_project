...
class PowerTask(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from power_project.power_project.doctype.power_task_depends_on.power_task_depends_on import PowerTaskDependsOn
	...
	def validate_parent_expected_end_date(self):
		...
		parent_exp_end_date = frappe.db.get_value("Power Task", self.parent_task, "exp_end_date")
	...
	def validate_parent_project_dates(self):
		...
		if project_end_date := frappe.db.get_value("Power Project", self.project, "expected_end_date"):
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if frappe.db.get_value("Power Task", d.task, "status") not in ("Completed", "Cancelled"):
					frappe.throw(
						_(
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not frappe.db.get_value("Power Task", self.parent_task, "is_template"):
	...
	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not frappe.db.get_value("Power Task", task.task, "is_template"):
	...
	def update_time_and_costing(self):
		tl = frappe.db.sql(
			...
			sum(hours) as time from `tabPower Timesheet Detail` where task = %s and docstatus=1""",
			self.name,
			as_dict=1,
		)[0]
		if self.status == "Open":
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			frappe.get_cached_doc("Power Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				tasks = frappe.db.sql(
					" select {} from `tabPower Task Depends On` where {} = {} ".format(d[0], d[1], "%s"),
		...
		if end_date:
			for task_name in frappe.db.sql(
				...
				where parent.project = %(project)s
					and parent.name in (
						select parent from `tabPower Task Depends On` as child
						where child.task = %(task)s and child.project = %(project)s)
			""",
				{"project": self.project, "task": self.name},
				as_dict=1,
			):
				task = frappe.get_doc("Power Task", task_name.name)
				if (
					task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = frappe.db.get_value(
			"Power Project User", {"parent": self.project, "user": frappe.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = frappe.get_doc("Power Task", self.parent_task)
...
def check_if_child_exists(name):
	...
	child_tasks = [get_link_to_form("Power Task", task.name) for task in child_tasks]
	return child_tasks
...
def get_project(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql(
		f""" select name {search_columns} from `tabPower Project`
...
def set_multiple_status(names, status):
	...
	for name in names:
		task = frappe.get_doc("Power Task", name)
		task.status = status
		task.save()
...
def add_multiple_tasks(data, parent):
	...
	new_doc = {"doctype": "Power Task", "parent_task": parent if parent != "All Tasks" else ""}
	new_doc["project"] = frappe.db.get_value("Power Task", {"name": parent}, "project") or ""
...
def on_doctype_update():
	frappe.db.add_index("Power Task", ["lft", "rgt"])
