...
class PowerProject(Document):
	...
	if TYPE_CHECKING:
		...
		from power_project.power_projects.doctype.power_project_user.power_project_user import PowerPowerProjectUser
		...
		users: DF.Table[PowerProjectUser]
	...
	def onload(self):
		self.set_onload(
			...
			from `tabPower Timesheet Detail` where project=%s and docstatus < 2 group by activity_type
	...
	def copy_from_template(self):
		...
		"""
		if self.project_template and not frappe.db.get_all("Power Task", dict(project=self.name), limit=1):
			# has a template, and no loaded tasks, so lets create
			if not self.expected_start_date:
				# project starts today
				self.expected_start_date = today()

			template = frappe.get_doc("Power Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = frappe.get_doc("Power Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for project_task in project_tasks:
			template_task = frappe.get_doc("Power Task", project_task.template_task)
	...
	def update_percent_complete(self):
		...
		total = frappe.db.count("Power Task", dict(project=self.name))
		...
		if not total:
			...
			if (self.percent_complete_method == "Task Completion" and total > 0) or (
				...
				completed = frappe.db.sql(
					"""select count(name) from `tabPower Task` where
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				...
				weighted_progress = frappe.db.sql(
					"""select progress, task_weight from `tabPower Task` where
					project=%s""",
					self.name,
					as_dict=1,
				)
	...
	def update_costing(self):
		...
		TimesheetDetail = frappe.qb.DocType("Power Timesheet Detail")
		from_time_sheet = (
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			frappe.db.set_value("Power Project", new_name, "copied_from", new_name)
...
def get_timeline_data(doctype: str, name: str) -> dict[int, int]:
	...
	timesheet_detail = frappe.qb.DocType("Power Timesheet Detail")
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	customers, suppliers = get_customers_suppliers("Power Project", frappe.session.user)
...
def allow_to_make_project_update(project, time, frequency):
	data = frappe.db.sql(
		""" SELECT name from `tabPower Project Update`
...
def send_project_update_email_to_users(project):
	doc = frappe.get_doc("Power Project", project)
...
def collect_project_status():
	for data in frappe.get_all("Power Project Update", {"date": today(), "sent": 0}):
		replies = frappe.get_all(
			"Communication",
			fields=["content", "text_content", "sender"],
			filters=dict(
				reference_doctype="Power Project Update",
		...
		for d in replies:
			doc = frappe.get_doc("Power Project Update", data.name)
...
def send_project_status_email_to_users():
	...
	for d in frappe.get_all("Power Project Update", {"date": yesterday, "sent": 0}):
		doc = frappe.get_doc("Power Project Update", d.name)

		project_doc = frappe.get_doc("Power Project", doc.project)
...
def update_project_sales_billing():
	...
	exists_query = "(SELECT 1 from `tab{doctype}` where docstatus = 1 and project = `tabPower Project`.name)"
	project_map = {}
	for project_details in frappe.db.sql(
		"""
			SELECT name, 1 as order_exists, null as invoice_exists from `tabPower Project` where
			exists {order_exists}
			union
			SELECT name, null as order_exists, 1 as invoice_exists from `tabPower Project` where
		...
		project = project_map.setdefault(
			project_details.name, frappe.get_doc("Power Project", project_details.name)
		)
...
def create_kanban_board_if_not_exists(project):
	...
	project = frappe.get_doc("Power Project", project)
...
def set_project_status(project, status):
	...
	for task in frappe.get_all("Power Task", dict(project=project.name)):
		frappe.db.set_value("Power Task", task.name, "status", status)
