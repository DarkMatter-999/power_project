...
def execute(filters=None):
	...
	for project in data:
		project["total_tasks"] = frappe.db.count("Power Task", filters={"project": project.name})
		project["completed_tasks"] = frappe.db.count(
			"Power Task", filters={"project": project.name, "status": "Completed"}
		)
		project["overdue_tasks"] = frappe.db.count(
			"Power Task", filters={"project": project.name, "status": "Overdue"}
		)
